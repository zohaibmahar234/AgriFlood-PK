...
.addTo(map)
.bindPopup("Khairpur Mir's study point")
.openPopup();
...
map.on('click',e=>{
  ...
  $('#mapDataNote').textContent=
    `Selected advisory coordinate: ${lat.toFixed(4)}, ${lon.toFixed(4)}`;
});
...
function dateLabel(){
  ...
  $('#todayLabel').textContent=d.toLocaleDateString(
    'en-GB',
    {
      weekday:'short',
      day:'2-digit',
...
async function loadHealth(){
  ...
  try{
    ...
    $('#geeStatus').textContent=
      h.gee_configured
        ? 'GEE configured'
        : 'GEE not configured';

    $('#geeStatus').style.color=
      h.gee_configured
        ? '#80efb0'
        : '#eacb77';
...
async function loadWeatherAndAdvisory(){
  ...
  try{
    ...
    $('#advisoryBox').textContent=
      'Current providers unavailable: '
      +e.message;
...
function renderWeather(w){
  ...
  $('#weatherBigIcon').textContent=icon;
  ...
  $('#topWeatherText').textContent=
    desc;

  $('#weatherDesc').textContent=
    desc;

  $('#humidityNow').textContent=
  ...
  $('#windNow').textContent=
    current.wind_speed_10m!=null
      ? `${fmt(current.wind_speed_10m,1)} km/h`
      : '—';

  $('#rainNow').textContent=
    `${fmt(w.rainfall_24h_mm,1)} mm`;

  $('#kpiRain').textContent=
    fmt(w.rainfall_24h_mm,1);
  ...
  $('#forecastRow').innerHTML=
    html ||
    '<div class="forecast-empty">Forecast fields unavailable from provider.</div>';
...
function renderAdvisory(a,d){
  ...
  $('#advisoryBox').innerHTML=`
    ...
    <span style="color:#7ea29a">
      Experimental rule ·
      not a calibrated flood probability.
    </span>
  `;
...
async function loadAlerts(){
  ...
  try{
    ...
    $('#recentAlerts').innerHTML=
      ...
      : '<div class="empty-state">No active alerts yet.</div>';
  ...
  return `
    <div class="alert-row">
      ...
      <div>
        ...
        <small>
          ${escapeHtml(a.source_date)}
          ·
          ${escapeHtml(a.quality)}
          ${
            a.simulated
              ? ' · SIMULATED'
              : ''
...
$('#runFlood').onclick=async()=>{
  ...
  $('#scienceMessage').textContent=
    'Processing Earth Engine request…';
...
/* =========================================================
   FLOOD RESULT + EARTH ENGINE MAP LAYER
   ========================================================= */
...
function renderFlood(d){
  ...
  if(d.status==='empty'){
    ...
    $('#scienceMessage').textContent=
      `No matching Sentinel-1 scenes. Before: ${d.before_scenes}, after: ${d.after_scenes}.`;
  ...
  $('#cropFloodExtent').textContent=
    `${fmt(
      d.potentially_affected_cropland_km2,
      1
    )} km²`;


  $('#sceneCount').textContent=
    `${d.before_scenes}+${d.after_scenes}`;


  /*
   Remove previous Earth Engine
   flood layer before adding
   the latest result.
  */
  ...
  /*
   ...
   candidate raster returned
   by FastAPI / Earth Engine.
  */
    ...
    */
    ...
    floodLayer.on(
      ...
      ()=>{
        ...
        $('#mapDataNote').textContent=
          'Flood result generated, but one or more Earth Engine map tiles could not be loaded.';
...
scientificLayerControl = L.control.layers(
  ...
  },
  {
    ...
    */
    ...
    $('#mapDataNote').textContent=
      'Cyan layer: experimental Sentinel-1 flood candidate areas.';
    ...
    $('#mapDataNote').textContent=
      'Flood summary loaded, but Earth Engine raster tile URL is unavailable.';

  }


  $('#scienceMessage').textContent=
    'Experimental Sentinel-1 flood candidate result; permanent-water and slope filters applied. Scientific validation pending.';
...
function renderRecovery(d){
  ...
  if(d.status==='empty'){
    ...
    $('#ndviChart').innerHTML=
      `<div class="chart-placeholder">
        No matching Sentinel-2 scenes.
        Baseline: ${d.baseline_scenes};
        recovery: ${d.recovery_scenes}.
      </div>`;
    ...
    if(view==='weather'){
      ...
      c.innerHTML=`
        ...
        <div class="report-note">
          ...
          It provides forecast/advisory context only
          and is not proof of future flooding.
        </div>
      `;
  ...
  },
  {
    ...
    /* ALERTS */
      ...
      c.innerHTML=
        ...
        : '<div class="empty-state">No alerts currently available.</div>';
    ...
    /* REPORTS */
      ...
      c.innerHTML=`
        ...
        <div class="detail-grid">
          ...
          ${
            metric(
              'Validation',
              'Pending',
              'Precision / Recall / F1 / IoU after reference comparison'
            )
          }
        ...
        <div class="report-note">
          Research reporting must keep software
          correctness separate from scientific accuracy.
          No scientific metric is shown until a suitable
          time-matched reference map is available.
        </div>
      `;
